...
class Entry(models.Model):
    ...
    commission_checked = models.NullBooleanField("Отметка комиссии", default=False, null=True)
...
class Member(models.Model):
    ...
    passport = models.CharField("Паспорт (серия, номер)", max_length=12, blank=True, null=True)
    passport_issued = models.CharField("Кем выдан", max_length=128, blank=True, null=True)
    issue_date = models.DateField("Дата выдачи", blank=True, null=True)
    address = models.CharField("Адрес", max_length=256, null=True)
    grade = models.CharField("Класс", max_length=3, null=True)
    phone_number = models.CharField("Тел.номер", max_length=12, null=True)
    email = models.CharField("email", max_length=64, null=True)
    ...
    def __str__(s):
        all_fields = list(filter(None, [str(s.id), '['+s.get_full_name(), s.grade+' класс', str(s.birthday)+']', s.phone_number, s.email, s.address,
...
class Work(models.Model):
    name = models.CharField("Название", max_length=128, null=True)
    theses = models.FileField("Тезисы", null=True)
